...
utrecht = utrecht.assign(
    quantile_values=pd.cut(
        utrecht.amount, quantile_bins, labels=quantile_labels_5[1:]
    )
    .astype(str)
    .replace("nan", quantile_labels_5[0])
...
color_dict = {i: j for i, j in zip(quantile_labels_5, colors)}


bar_frame = (
    utrecht.quantile_values.value_counts()
    .reindex(quantile_labels_5[::-1])
    .reset_index()
...
ax_child.set_yticks([0, 1, 2, 3, 4, 5], labels=quantile_labels_5[::-1], fontweight="bold", color=text_color)
...
# fig.savefig("utrecht2.png", dpi=1200)
